...
double RanMars::Halfgaussian(double mu, double sigma)
{
  double v1;
  v1 = abs(mu+sigma*gaussian());
  return v1;
}
...
double RanMars::lognormal(double mu, double sigma)
{
  double vv;
  vv = exp(mu+sigma*gaussian());
  return vv;
}
...
double RanMars::onesideLevy(double c, double alpha)
{
  double V, W, X;
  V = MathConst::MY_PI*(uniform()-0.5);
  W = uniform();
  double B = MathConst::MY_PI2 ;
  double t = tan (B* alpha);
  double S =(pow,(1 + t * t, 1/(2 * alpha)));
  X = S * sin (alpha * (V + B)) /(pow ,(cos (V), 1 / alpha))
    *(pow, (cos (V - alpha * (V + B)) / (-log(W)), (1 - alpha) / alpha));
  return c * X;
...
double RanMars::maxboltz(double sigma)
{
  double u1, u2, u3, X;
  u1 = gaussian();
  u2 = gaussian();
  u3 = gaussian();
  X = sqrt(u1*u1 + u2*u2 + u3*u3);
  return sigma * X;
}
...
double RanMars::gengamma(double shape, double etta, double scale)
{
 ...
 //  {
   ...
   if (u < 1.0 -0.0331*xsquared*xsquared || log(u) < 0.5*xsquared + d*(1.0 - v + log(v)))
    printf("this are the control vals %g %g %g %g %g %g %g \n", d,c,v,u,shape,etta,scale);
   return scale*(pow(d*v, 1.0/etta));
   //}
  }
  else if (shape <= 0.0) error->all(FLERR,"Shape parameter must be positive."); 
  else
  {
   double g = gengamma(shape+1.0, 1.0, 1.0);
   double w = uniform();

   return scale*(pow(g*(pow(w, 1.0/shape)),1.0/etta));
  }
}

...
enum{TYPE,TYPE_FRACTION,TYPE_RATIO,TYPE_SUBSET,
     ...
     SMD_CONTACT_RADIUS,DPDTHETA,INAME,DNAME,VX,VY,VZ, DPDGAMMA};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      set(SPH_RHO);
      iarg += 2;
      /////////////MODIFIED BY NMC TO SET GAMMA
    } else if (strcmp(arg[iarg],"difdif/gamma") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      if (!atom->gammai_flag)
        error->all(FLERR,"Cannot set difdif/gamma for this atom style");
      set(DPDGAMMA);
      iarg += 2;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    //Modified by NMC
    else if (keyword == DPDGAMMA) atom->gammai[i] = dvalue;

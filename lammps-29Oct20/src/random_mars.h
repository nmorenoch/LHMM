...
class RanMars : protected Pointers {
 public:
  ...
  double Halfgaussian(double mu, double sigma);
  double onesideLevy(double c, double alpha);
  double maxboltz(double sigma);
  double rayleigh(double sigma);
  double besselexp(double theta, double alpha, double cp);
    double gengamma(double shape, double etta, double scale);

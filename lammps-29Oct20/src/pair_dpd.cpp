...
void PairDPD::compute(int eflag, int vflag)
{
  ...
  //New variables for modified friction coefficient
  double gamvar, sigvar,sGamma;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        //new
        gamvar=0;
        if (disType==0) gamvar = 0.5*(gamma[itype][jtype]);//gamma[itype][jtype];
        if (disType==1) gamvar = random->Halfgaussian(gamma[itype][jtype],sGamma);
        if (disType==2) gamvar = random->lognormal(gamma[itype][jtype], sGamma);
        if (disType==3) gamvar = random->onesideLevy(1.0, gamma[itype][jtype]);
        if (disType==4) gamvar = random->maxboltz(gamma[itype][jtype]);
        

        sigvar = sqrt(2.0*force->boltz*temperature*gamvar); //sigma[itype][jtype]
            

        fpair -= gamvar*wd*wd*dot*rinv;
...
void PairDPD::settings(int narg, char **arg)
{
  ...
  if (disType>4) error->all(FLERR, "Illegal number of distribution type");  
